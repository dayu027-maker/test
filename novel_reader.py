    ...
    def worker_loop(self):
        while self.running:
            ...
            if cmd == "speak":
                ...
                if self._is_darwin:
                    try:
                        # macOS: 先生成音频文件，再用 afplay 播放以支持音量控制
                        tf = tempfile.mktemp(prefix='tts_', suffix='.aiff')
                        
                        # 1. 生成 (say)
                        args_gen = ["say", "-o", tf, "-r", str(self.rate), text]
                        self._proc = subprocess.Popen(args_gen)
                        ret = self._proc.wait()
                        
                        # 2. 播放 (afplay) - 仅当生成成功且未被终止时
                        if ret == 0 and os.path.exists(tf):
                            # volume: 0.0-1.0 (afplay default 1, allows float)
                            # afplay -v <volume>
                            args_play = ["afplay", "-v", str(self.volume), tf]
                            self._proc = subprocess.Popen(args_play)
                            self._proc.wait()
...
class NovelReaderApp:
    ...
    def _build_ui(self):
        # 绑定快捷键
        self.root.bind('<n>', self._page_down)
        self.root.bind('<m>', self._page_up)
        self.root.bind('<N>', self._page_down)
        self.root.bind('<M>', self._page_up)
        ...
        # 翻页按钮
        tk.Button(top_toolbar, text="◀ 上一页", command=self._page_up).pack(side=tk.LEFT, padx=6)
        tk.Button(top_toolbar, text="下一页 ▶", command=self._page_down).pack(side=tk.LEFT, padx=(0, 6))
        ...
        # 定时停止设置
        timer_frame = tk.Frame(top_toolbar)
        timer_frame.pack(side=tk.RIGHT, padx=6)
    ...
    def _apply_theme(self, theme):
        try:
            ...
            # 确保高亮文字在夜间模式下（以及日间模式）都是黑色，以便在黄色背景上清晰可见
            try:
                self.text.tag_configure("reading_highlight", foreground="black", background="#efefef")
            except Exception:
                pass
        except Exception:
            pass
    ...
    def start_timer_stop(self):
        ...
        if not self.timer_stop_enabled or not self.reading:
            return
        ...
        self.timer_stop_minutes = minutes
        self.timer_remaining_seconds = minutes * 60  # 剩余秒数
    ...
    def cancel_timer_stop(self):
        ...
        # 取消倒计时显示更新定时器
        if hasattr(self, 'timer_display_after_id') and self.timer_display_after_id:
            self.root.after_cancel(self.timer_display_after_id)
            self.timer_display_after_id = None
        self.timer_status_label.config(text="")

    def _update_timer_display(self):
        """更新倒计时显示"""
        if not self.reading or not self.timer_stop_enabled:
            return
        
        # 格式化显示时间
        minutes = self.timer_remaining_seconds // 60
        seconds = self.timer_remaining_seconds % 60
        self.timer_status_label.config(text=f"还剩{minutes:02d}:{seconds:02d}")
        ...
        if self.timer_remaining_seconds >= 0:
            self.timer_display_after_id = self.root.after(1000, self._update_timer_display)
    ...
    def start_reading(self):
        ...
        try:
            top_index = self.text.index('@0,0')
            start_line = int(top_index.split('.')[0])
        except Exception:
            # Fallback
            start_line = 1
